...
KeyboardLayout(){
	...
	IfInString, InputLocaleID, 409
		Return "English"
	IfInString, InputLocaleID, 401
		Return "Arabic"
	Return "Other"


	the_language := languageCode_%A_Language%  ; Get the name of the system's default language.
	;Return %A_Language%  ; Display the language name.	
	
	MsgBox, % "sdfa " 0x0409 & 0xFFFF
...
IsKeyboardArabic(){
	lo := KeyboardLayout()
	IfInString, lo, Arabic
		Return True
	Return False
}
IsKeyboardEnglish(){
	lo := KeyboardLayout()
	IfInString, lo, English
		Return True
	Return False
}
SetKeyboardArabic(){
	SetInputLang(0x4010C01)
}
SetKeyboardEnglish(){
	SetInputLang(0x4090809)
}
SetDefaultKeyboard(LocaleID){ ;Changes system language
